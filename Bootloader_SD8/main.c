...
// IRAM1, IRAM2の設定については
// https://devzone.nordicsemi.com/question/45894/ota-dfu-bootloader-sdk900/
// を参照。
// IRAM1 0x20002c00 size 0x1380
// IRAM2 0x20003F80 size 0x80
...
/*
 * アプリケーションと共有しているEEPROMフラグ
 */
// I2Cデバイスのアドレス
#define EEPROM_I2C_ADDRESS          0xa0
// 上位4ビットはI2Cアドレス、続く2ビットはチップの配線で決まるID、次の1ビットは最上位アドレスビット、最後の1ビットはR/Wフラグ。
#define EEPROM_I2C_B16_BIT          0x02
#define EEPROM_ADDRESS_B16_MASK     0x00010000
...
bool readEEPROMData(uint32_t eeprom_address, uint8_t *data, uint8_t data_length)
{
    // I2Cスレーブアドレスを組み立てる。
    // 上位4ビットはI2Cアドレス、続く2ビットはチップの配線で決まるID、次の1ビットは最上位アドレスビット、最後の1ビットはR/Wフラグ。
    uint8_t slave_address = EEPROM_I2C_ADDRESS;
    if( (eeprom_address & EEPROM_ADDRESS_B16_MASK) != 0 ) {
        slave_address |= EEPROM_I2C_B16_BIT;
    }
    
    // EEPROMアドレスを組み立てる。EEPROM上位アドレスを配列[0]に、下位アドレスを[1]に入れる。
    uint8_t address[2];
    address[0] = (uint8_t)((eeprom_address >> 8) & 0x00ff);
    address[1] = (uint8_t)((eeprom_address >> 0) & 0x00ff);
    
    // リトライつきのEEPROM読み出し
    bool result = true;
    for(int i=0; i < 8; i++) {
        result = twi_master_transfer(slave_address, address, 2, false);
        result &= twi_master_transfer((slave_address | TWI_READ_BIT), data, data_length, true);
        if(result) {
            break;
        }
    }
...
bool readFlagOfEEPROM(uint32_t address)
{
    uint8_t data = 0;
    readEEPROMData(address, &data , 1);
    
    return (data == 1);
}

bool getDFUFlagFromEEPROM(void)
{
