...
// MPU9250に書き込みます。
// TWI_MPU9500_ADDRESS は senstick_io_definitions.h で定義されているI2Cバスのアドレスです。
static void writeToMPU9500(mpu92500_context_t *p_context, MPU9500Register_t target_register, const uint8_t *data, uint8_t data_length)
{
    ret_code_t err_code;
    ...
    // I2C書き込み
    err_code = nrf_drv_twi_tx(p_context->p_twi, TWI_MPU9500_ADDRESS, buffer, (data_length + 1), false);
    APP_ERROR_CHECK(err_code);
}

// MPU9250から読み込みます。
static void readFromMPU9500(mpu92500_context_t *p_context, MPU9500Register_t target_register, const uint8_t *data, uint8_t data_length)
{
    ret_code_t err_code;
    
    // 読み出しターゲットアドレスを設定
    uint8_t buffer0 = (uint8_t)target_register;
    err_code = nrf_drv_twi_tx(p_context->p_twi, TWI_MPU9500_ADDRESS, &buffer0, 1, true);
    APP_ERROR_CHECK(err_code);

    // データを読み出し
    err_code = nrf_drv_twi_rx(p_context->p_twi, TWI_MPU9500_ADDRESS, data, data_length, false);
    APP_ERROR_CHECK(err_code);
}

static void getAccelerationData(mpu92500_context_t *p_context, AccelerationData_t *p_acceleration)
{
    uint8_t buffer[6];
    ...
    // ACCEL_ZOUT_H
    // ACCEL_ZOUT_L
    readFromMPU9500(p_context, ACCEL_XOUT_H, buffer, sizeof(buffer));
...
static void getRotationRateData(mpu92500_context_t *p_context, RotationRateData_t *p_rotaionRate)
{
    uint8_t buffer[6];
    ...
    // GYRO_ZOUT_H
    // GYRO_ZOUT_L
    readFromMPU9500(p_context, GYRO_XOUT_H, buffer, sizeof(buffer));
...
getMagneticFieldData(mpu92500_context_t *p_context, MagneticFieldData_t *p_magneticField)
{
    uint8_t buffer[6];
    ...
    // HZL
    // HZH
    readFromMPU9500(p_context, HXL, buffer, sizeof(buffer));
...
void initNineAxesSensor(nine_axes_sensor_context_t *p_context, nrf_drv_twi_t *p_twi)
{
    ...
    writeToMPU9500(p_context, PWR_MGMT_1, data, sizeof(data));
    ...
    writeToMPU9500(p_context, CNTRL1, data1, sizeof(data1));
}
...
void sleepNineAxesSensor(nine_axes_sensor_context_t *p_context)
{
    ...
    writeToMPU9500(p_context, PWR_MGMT_2, data, sizeof(data));
    ...
    writeToMPU9500(p_context, PWR_MGMT_1, data2, sizeof(data2));
    ...
    writeToMPU9500(p_context, CNTRL1, data3, sizeof(data3));
}
...
void getNineAxesData(nine_axes_sensor_context_t *p_context, MotionSensorData_t *sensor_data)
{
    ...
    getRotationRateData(p_context,  &(sensor_data->magneticField));
    getMagneticFieldData(p_context, &(sensor_data->rotaionRate));
